                    ...
                    $(document).on('mousedown', '.search-button-youtube', function () {
                        ...
                        if (input && isUrl && isYoutube) {
                            ...
                            if (url.match(/&/)) {
                                ...
                                jsonYt = !config.youtube.API_KEY ? 0 : ajaxDataVideo(videoID);
                                isVideo = true;
                            } else {
                                ...
                                if (url.match(/list=/)) {
                                    ...
                                    jsonYt = ajaxDataPlaylistItem(listID);
                                    isVideo = false;
                                } else if (url.match(/v=/)) {
                                    videoID = url.split('v=')[1];
                            ...
                            if (jsonYt == -1) tabName = 'The API Key is missing; the key is needed to show a name of the now-playing video properly.';
                        ...
                        if (input && isUrl && isYoutube) {
                            ...
                            if (url.match(/&/)) {
                                ...
                                if (url.match(/list=/)) {
                                    listID = url.split('list=')[1];
                                    youtubeID = 'videoseries?list=' + listID;
                                    jsonYt = !config.youtube.API_KEY ? 0 : ajaxDataPlaylistItem(listID);
                                    isVideo = false;
                                } else if (url.match(/v=/)) {
                                    videoID = url.split('v=')[1];
                                    youtubeID = url.split('v=')[1] + '?rel=0&showinfo=0';
                            ...
                            var test = tabName;
                            $(this).parents('.youtube-wrapper').find('.tab-youtube').text(test);
                        } else {}
                    });
                };
                searchBox();
